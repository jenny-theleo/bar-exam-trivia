...
// ── TOPIC PICKER (shown to the round loser after each round) ──────────────────
function TopicPickerScreen({ pickerName, onTopicChosen }) {
  return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "system-ui, sans-serif" }}>
      <div style={{ maxWidth: 420, width: "100%", textAlign: "center" }}>
        <div style={{ fontSize: 44, marginBottom: 16 }}>🎯</div>
        <h2 style={{ color: C.text, fontSize: 26, fontWeight: 900, margin: "0 0 8px" }}>Pick the Next Topic</h2>
        <p style={{ color: C.muted, fontSize: 14, marginBottom: 32 }}>
          <span style={{ color: C.p2, fontWeight: 700 }}>{pickerName}</span> lost the round — you pick the topic!
        </p>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {TOPIC_NAMES.map(topic => (
            <button key={topic} onClick={() => onTopicChosen(topic)}
              style={{
                background: TOPICS[topic].color + "18",
                border: `2px solid ${TOPICS[topic].color}55`,
                borderRadius: 16,
                padding: "20px 24px",
                cursor: "pointer",
                textAlign: "left",
              ...
              <span style={{ fontSize: 32 }}>{TOPICS[topic].icon}</span>
              <div>
                <div style={{ color: TOPICS[topic].color, fontWeight: 800, fontSize: 18, fontFamily: "system-ui, sans-serif" }}>{topic}</div>
                <div style={{ color: C.muted, fontSize: 12, marginTop: 2 }}>{TOPICS[topic].questions.length} questions in pool</div>
              </div>
            </button>
          ))}
        </div>
...
// ── WAITING FOR OPPONENT TOPIC PICK ──────────────────────────────────────────
    ...
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "system-ui, sans-serif" }}>
      <div style={{ maxWidth: 420, width: "100%", textAlign: "center" }}>
        ...
        <p style={{ color: C.muted, fontSize: 14, marginBottom: 24 }}>
          <span style={{ color: C.p2, fontWeight: 700 }}>{loserName}</span> is picking the next topic…
        </p>
...
function QuestionScreen({ question, questionNum, totalQuestions, onAnswer, timeLeft, answered, topic }) {
      ...
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "14px 20px" }}>
        ...
        <div style={{ maxWidth: 560, margin: "6px auto 0" }}>
          <div style={{ height: 3, background: C.border, borderRadius: 2 }}>
            <div style={{ height: "100%", width: `${(timeLeft / 60) * 100}%`, background: timerColor, borderRadius: 2, transition: "width 1s linear, background 0.3s" }} />
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "24px 16px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {question.answers.map((ans, i) => {
            ...
            return (
              <button key={i} onClick={() => handleSelect(letter)}
                disabled={!!selected || answered}
                style={{
                  background: isSelected ? topicConfig.color + "22" : C.surface,
                  border: `2px solid ${isSelected ? topicConfig.color : C.border}`,
                  borderRadius: 12, padding: "14px 16px", cursor: (selected || answered) ? "default" : "pointer",
                  textAlign: "left", display: "flex", gap: 12, alignItems: "flex-start", transition: "all 0.15s"
                }}>
                <span style={{ background: isSelected ? topicConfig.color : C.border, color: "#fff", borderRadius: 6, width: 22, height: 22, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, flexShrink: 0, marginTop: 1, fontFamily: "monospace" }}>{letter}</span>
                <span style={{ color: C.text, fontSize: 14, lineHeight: 1.5 }}>{ans.replace(/^\([A-D]\)\s*/, "")}</span>
              </button>
            );
          })}
        </div>
        ...
        {selected && (
          <div style={{ marginTop: 20, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, textAlign: "center" }}>
            <div style={{ color: C.muted, fontSize: 14 }}>Answer locked in. Waiting for your opponent to answer…</div>
            <div style={{ marginTop: 8, display: "flex", justifyContent: "center", gap: 6 }}>
              {[0, 1, 2].map(i => (
                <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: topicConfig.color, animation: `pulse ${0.6 + i * 0.2}s ease-in-out infinite alternate`, opacity: 0.7 }} />
              ))}
            </div>
          </div>
        )}
...
export default function App() {
  ...
  const handleCreateGame = async (name) => {
    ...
    const firstTopic = "Constitutional Law";
    const firstQs = drawRoundQuestions(firstTopic, []);
    const room = {
      code,
      creator: name,
      joiner: null,
      roundQuestions: { 0: firstQs.map(q => q.id) },
      roundTopics: { 0: firstTopic },
      usedIds: { "Constitutional Law": firstQs.map(q => q.id), "Evidence": [] },
      answers: {},
      round: 0,
      topicPick: null, // set when loser picks next topic
      createdAt: Date.now(),
    };
    ...
    setUsedIds({ "Constitutional Law": firstQs.map(q => q.id), "Evidence": [] });
    setScreen("playing");
    startQuestion(0, 0);
  ...
  const handleAnswer = async (letter, round, qIdx) => {
    ...
    const myKey = `${isCreator ? "creator" : "joiner"}_${key}`;
    const newMyAnswers = { ...myAnswers, [key]: letter };
    setMyAnswers(newMyAnswers);

    const room = await loadRoomData(roomCode);
    if (!room) return;
    room.answers[myKey] = letter;
    await saveRoomData(roomCode, room);

    setWaitingForOpponent(true);
    pollForOpponentAnswer(round, qIdx, newMyAnswers);
  };

  // ── POLL FOR OPPONENT ANSWER ──────────────────────────────────────────────
  const pollForOpponentAnswer = (round, qIdx, currentMyAnswers) => {
    const key = qKey(round, qIdx);
    const oppKey = `${isCreator ? "joiner" : "creator"}_${key}`;

    pollRef.current = setInterval(async () => {
      const room = await loadRoomData(roomCode);
      if (!room) return;

      if (room.answers[oppKey]) {
        clearInterval(pollRef.current);
        const oppAnswer = room.answers[oppKey];
        const newOppAnswers = { ...opponentAnswers, [key]: oppAnswer };
        setOpponentAnswers(newOppAnswers);

        const nextQ = qIdx + 1;
        if (nextQ < QUESTIONS_PER_ROUND) {
          setWaitingForOpponent(false);
          startQuestion(round, nextQ);
        } else {
          // Build round results
          const results = [];
          for (let i = 0; i < QUESTIONS_PER_ROUND; i++) {
            ...
            const myAns = currentMyAnswers[k] || myAnswers[k];
            const theirAns = room.answers[`${isCreator ? "joiner" : "creator"}_${k}`];
            results.push({
              correct: q ? q.correct : "?",
              myAnswer: myAns,
  ...
  if (screen === "topicPicker") {
    return <TopicPickerScreen pickerName={playerName} onTopicChosen={handleTopicChosen} />;
  }
  ...
  if (screen === "playing") {
    ...
    if (!q) return null;
    const myKey = qKey(currentRound, currentQ);
    const alreadyAnswered = !!myAnswers[myKey];
    return (
      <div style={{ position: "relative" }}>
        ...
        <div style={{ paddingTop: showCodeBanner ? 80 : 0 }}>
          <QuestionScreen
            ...
            timeLeft={timeLeft}
            answered={alreadyAnswered || waitingForOpponent}
            topic={currentTopic}
          />
        </div>
      </div>
